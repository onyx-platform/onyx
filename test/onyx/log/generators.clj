...
;; Diff generator actions
;; Generate peer-id / action tuples
;; Update to current replica entry, and apply new entry
;; Stay on whatever replica peer is on, and play message
;; Update to current replica entry, and write message

;; Other issues:
;; If we're going to call fire-side-effects! we're going to need 
;; to prevent:
;; close!
;; >!!
;; Thread/sleep
;; monitoring emit should no-op


; These from leave-cluster 
...
; These from notify join cluster
...
; Similar from prepare join cluster
...
(defn apply-entry [replica entries entry]
  (let [new-replica (extensions/apply-log-entry entry replica)
        ...
        peer-reactions (keep (fn [peer-id]
                               (let [state (assoc base-peer-state :id peer-id)] 
                                 (if-let [reactions (seq (extensions/reactions entry replica new-replica diff state))]
                                   [peer-id reactions])))
                             peers)
