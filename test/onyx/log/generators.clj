...
(defn active-groups [replica entry]
  (cond-> (set (concat (:groups replica)
    ...
    (get #{:prepare-join-cluster :accept-join-cluster :abort-join-cluster :notify-join-cluster :group-leave-cluster} (:fn entry))
    (into (remove nil? ((juxt :observer :id :accepted-observer :accepted-joiner :joiner) (:args entry))))))

(defn active-peers [replica entry]
  (cond-> (:peers replica)
    (get #{:add-virtual-peer :leave-cluster} (:fn entry))
    (conj (:id (:args entry)))))
