...
(deftest state-backend-differences
  (checking "Memory db as oracle for state db"
   ...
   (let [db-name (str (java.util.UUID/randomUUID))
         ...
         mem-store (onyx.state.memory/create-db {} :state-id-1 coders)
         db-store (onyx.state.lmdb/create-db {} db-name coders)
         cp-encoder-db (cpenc/empty-checkpoint-encoder)
         cp-encoder-mem (cpenc/empty-checkpoint-encoder)
         db-store-memory->lmdb (onyx.state.lmdb/create-db {} (str (java.util.UUID/randomUUID)) coders)
         db-store-lmdb->memory (onyx.state.memory/create-db {} (str (java.util.UUID/randomUUID)) coders)]
     (try
      ;time 
