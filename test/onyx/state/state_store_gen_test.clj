...
(def window-generators
  (gen/one-of [; FIXME
               ...
               gen-global-grouped gen-sliding-grouped 
               gen-global-ungrouped gen-sliding-ungrouped 
               ]))
  ...
  (checking "Memory db as oracle for state db"
            ...
            (let [db-name (str (java.util.UUID/randomUUID))
              ...
              (try
               ...
               (doseq [example values]
                 (case (first example) 
                   ...
                   (let [[_ [state-idx window-type group-key extent time] value] example]
                     (s/put-state-entry! db-store state-idx (when group-key (s/group-id db-store group-key)) time value)
                     (s/put-state-entry! mem-store state-idx (when group-key (s/group-id mem-store group-key)) time value))
                   :delete-time-entries
                   (let [[_ [state-idx _ group-key extent] start end] example]
                     (s/delete-state-entries! db-store state-idx (when group-key (s/group-id db-store group-key)) start end)
                     (s/delete-state-entries! mem-store state-idx (when group-key (s/group-id mem-store group-key)) start end))
               ...
               #_(is (= (sort (remove nil? (s/groups db-store)))
                        (sort (remove nil? (s/groups mem-store)))
                      #_(sort (remove nil? (s/groups db-store-memory->lmdb state-idx)))
                      #_(sort (remove nil? (s/groups db-store-lmdb->memory state-idx)))))


               (doseq [[state-idx group-key] (->> values
                                                  (filter (fn [[type]]
                                                            (not (#{:add-trigger :add-time-entry :delete-time-entries} type))))
                 ...
                 (let [serialized-group (some->> group-key (s/group-id db-store))
                   ...
                   #_(is (= (s/get-trigger db-store state-idx serialized-group)
                   ...
                   #_(is (= (s/group-extents db-store state-idx serialized-group)
               ...
               (finally 
                (s/drop! db-store)
                (s/close! db-store)
                #_(s/drop! db-store-memory->lmdb)
                #_(s/close! db-store-memory->lmdb)
                #_(s/drop! db-store-lmdb->memory)
                #_(s/close! db-store-lmdb->memory)
                (s/drop! mem-store)
                (s/close! mem-store))))))
