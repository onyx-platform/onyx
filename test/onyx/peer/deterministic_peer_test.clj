...
(with-redefs [onyx.peer.task-lifecycle/backoff-until-task-start! (fn [_])
  ...
  (let [n-messages 100
        ...
        _ (println "discovered " discovered-tasks)
        ;; TODO, do this later
        ...
        new-replica (reduce (fn [replica entry] 
                            ...
                            replica
                            log)
        _ (println "hnew replcia " new-replica)

        job-id (first (:jobs new-replica))

        _ (println "starting" (get-in new-replica [:allocations job-id]))
