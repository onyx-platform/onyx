(ns onyx.peer.short-circuit-test
...
(deftest ^:smoke short-circuit-test
  (let [id (random-uuid)
    ...
    (with-expect-call [(:do coordinator/periodic-barrier [_])
      ...
      (with-test-env [test-env [3 env-config (assoc peer-config :onyx.messaging/allow-short-circuit? true)]]
        (let [batch-size 20
              catalog [{:onyx/name :in
                       ...
                       {:onyx/name :inc
                        :onyx/fn :onyx.peer.short-circuit-test/my-inc
                        :onyx/batch-size 9
                        :onyx/type :function}
