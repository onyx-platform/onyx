...
Official plugins are vetted by Michael Drogalis. Ensure in your project that plugin versions directly correspond to the same Onyx version (e.g. `onyx-core-async` version `0.4.1` goes with `onyx` version `0.4.1`). Fixes to plugins can be applied using a 4th versioning identifier (e.g. `0.4.1.1`).
