...
```
[org.onyxplatform/onyx "0.7.12-SNAPSHOT"]
```
...
### API Docs 0.7.12-SNAPSHOT

Code level API documentation [can be found here](http://www.onyxplatform.org/api/0.7.12-SNAPSHOT).

### Official plugin listing

Official plugins are vetted by Michael Drogalis. Ensure in your project that plugin versions directly correspond to the same Onyx version (e.g. `onyx-core-async` version `0.7.12-SNAPSHOT` goes with `onyx` version `0.7.12-SNAPSHOT`). Fixes to plugins can be applied using a 4th versioning identifier (e.g. `0.7.12-SNAPSHOT.1`).
