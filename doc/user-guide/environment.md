...
- [ ] **Setup an external Aeron Media Driver**: If messaging performance is a factor, it is recommended that the Aeron Media Driver is run out of process. First, disable the embedded driver by setting `:onyx.messaging.aeron/embedded-driver? false`. An example out of process media driver is included in [lib-onyx](https://github.com/onyx-platform/lib-onyx/blob/master/src/lib_onyx/media_driver.clj). This media driver can be started by `:aot` compiling the `lib-onyx.media-driver` namespace and invoking it with `java -cp <uberjar> lib-onyx.media-driver <opts>`.
