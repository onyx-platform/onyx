...
setting `:onyx/bulk?` to `true` in your catalog entry for your function.
Onyx will _ignore_ the output of your function and pass the same
...
Functions with this key enabled may _not_ be used with flow conditions.
These segments are passed to all immediate downstream tasks.
