...
function is whenever a segment at the input task has been fully acked.
...
return of an input plugin ack-segment call, and the matching lifecycle
map. May return a value of any type which will be discarded. This
function is whenever a segment at the input task has been pending for
...
the details on the
www.onyxplatform.org/docs/cheat-sheet/latest/=lifecycle-calls/:lifecycle/handle-exception[Onyx
cheat sheet].
