         ...
         (loop []
           (if (.hasNext iterator)
             (let [entry ^Entry (.next iterator)]
               ...
               (recur))))
         ;; TODO: shouldn't need to sort this if comparator is right
         (sort (persistent! vs)))
       (finally 
        (.abort txn)))))
  (drop! [this]
...
(defn create-db
  ...
  (let [max-size 1024000
        ...
        env (doto (Env. path)
              (.setMapSize max-size))
        ;; Should be setting MDB_WRITEMAP, possibly MDB_NOSYNC, though this may lead to consistency problems
