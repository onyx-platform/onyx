...
(defn process-state-loop
  [{:keys [onyx.core/state-ch onyx.core/compiled
           onyx.core/task-information onyx.core/peer-opts] :as event} ex-f]
  (try 
    (let [timer-resolution (arg-or-default :onyx.peer/trigger-timer-resolution peer-opts)] 
      (loop [timer-tick-ch (timeout timer-resolution)]
        (let [[[event-type task-event ack-batch] ch] (alts!! [timer-tick-ch state-ch] :priority true)] 
          (cond (= ch state-ch)
                (when event-type 
                  (lc/invoke-assign-windows process-state compiled task-information (new-state-event event-type task-event))
                ...
                (= ch timer-tick-ch)
                (do 
                  (lc/invoke-assign-windows process-state compiled task-information (new-state-event :timer-tick event))
