...
(defn recover-windows
  [{:keys [onyx.core/windows onyx.core/triggers onyx.core/task-id onyx.core/slot-id onyx.core/task-map] :as event}
   ;; FIXME. NUM 1 CREATE NEW STATE STORE IN HERE? RATHER THAN DROP
   ;; 2. preprocess windows, get :idx, grab extent-serializer, we know grouped or not, 
   state-store
   recover-coordinates]
    ...
    (run! (fn [[coordinates mappings]]
            (let [bs (read-checkpoint event :windows coordinates slot-id)
                  ...
                  _ (when-not (= schema-version cpenc/current-schema-version)
                      (throw (ex-info "Incompatible schema for state checkpoint." {:current cpenc/current-schema-version
                                                                                   :retrieved schema-version})))
                  metadata (checkpoint-decompress (cpenc/get-metadata decoder))
